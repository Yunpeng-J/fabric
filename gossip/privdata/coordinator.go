...
func (c *coordinator) StoreBlock(block *cached.Block, privateDataSets util.PvtDataCollections) chan error {
	errOut := make(chan error, 1)
	if block.Data == nil {
		errOut <- errors.New("Block data is empty")
		return errOut
	}
	if block.Header == nil {
		errOut <- errors.New("Block header is nil")
		return errOut
	}
	...
	dataOut := make(chan *ledger.BlockAndPvtData, 1)
	pvtInfoChan := make(chan *privateDataInfo, 1)
	go func() {
		...
		dataOut <- blockAndPvtData
		pvtInfoChan <- privateInfo
		errOut <- nil
	}()

	errReturn := make(chan error, 1)
	go func() {
		var blockAndPvtData *ledger.BlockAndPvtData
		var privateInfo *privateDataInfo
		err := <-errOut
		if err != nil {
			errReturn <- err
			return
		...
		blockAndPvtData = <-dataOut
		blockAndPvtData.UnblockedTxs = unblockedTx
		privateInfo = <-pvtInfoChan
		...
		err = c.CommitWithPvtData(blockAndPvtData, committedTxs)
		if err != nil {
			errReturn <- err
			return
		}
...
func forEachTxn(block *cached.Block, txsFilter txValidationFlags, consumer blockConsumer) (txns, error) {
	var txList []string
	envs, err := block.UnmarshalAllEnvelopes()
	if err != nil {
		logger.Warning("Invalid envelope:", err)
	}
	for seqInBlock, env := range envs {
		payload, err := env.UnmarshalPayload()
		if err != nil {
		...
		ccActionPayload, err := tx.UnmarshalChaincodeActionPayload()
		if err != nil {
		...
		txRWSet, err := cca.UnmarshalRwSet()
		if err != nil {
			logger.Warning("Failed obtaining TxRwSet from ChaincodeAction's results", err)
			continue
		}
		err = consumer(uint64(seqInBlock), chdr.ChannelHeader, txRWSet, ccActionPayload.Action.Endorsements)
		if err != nil {
			return txList, err
		}
	}
...
func (c *coordinator) listMissingPrivateData(block *cached.Block, ownedRWsets map[rwSetKey][]byte) (*privateDataInfo, error) {
	...
	txList, err := forEachTxn(block, txsFilter, bi.inspectTransaction)
	if err != nil {
		return nil, err
	}
...
func (c *coordinator) GetPvtDataAndBlockByNum(seqNum uint64, peerAuthInfo common.SignedData) (*cached.Block, util.PvtDataCollections, error) {
	...
	forEachTxn(blockAndPvtData.Block, make(txValidationFlags, len(data)), func(seqInBlock uint64, chdr *common.ChannelHeader, txRWSet *cached.TxRwSet, _ []*peer.Endorsement) error {
