...
func (r *REPL) Run(line string) {
	...
	code := obj.(*py.Code)
	_, err = vm.RunWithGas(r.module.Globals, r.module.Globals, code, nil, r.gas)
	if err != nil {
		if err.Error() == "Ran out of gas"{
			fmt.Println("setting to out of gas")
			r.OutOfGas = true
			py.TracebackDump(err.Error())
			return
		}
...
func (r *REPL) RunWithGas(s string, gas int) {
	r.gas = gas
	r.gasReset = false
	r.Run(s)
}
